...
let
  ...
  inherit (lib) mkForce;
  mkSystemSimple = { description, s_after ? [ "network-online.target" ], s_wants ? [ "network-online.target" ], exec, preStart ? "", postStop ? "", timeout ? 300 }: { # Helper template
...
in
{
  ...
  users = {
    users."${user.name}" = {
      ...
      home = "/home/${user.name}";
      hashedPasswordFile = "/etc/nixos/secret-password";
    };
  };
  ...
  systemd = {
    services = { # https://discourse.nixos.org/t/screen-locker-crashing/33510
      ...
      deploy-vm-network = mkSystemSimple {
        description = "Automatic deployment of vm network";
        exec = "${pkgs.bash}/bin/bash /etc/nixos/vms/deploy-vm-network.sh";
      };

      deploy-colt-vm = mkSystemSimple {
        description = "Automatic deployment of colt vm";
        s_after = [ "deploy-vm-network.service" ];
        s_wants = [ "deploy-vm-network.service" ];
        exec = "${pkgs.bash}/bin/bash /etc/nixos/vms/deploy-colt-vm.sh";
      };
    };
  };
